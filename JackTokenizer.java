...
public class JackTokenizer {
    ...
    /**
     * Enum for all the possible token types in Jack.
     */
     ...
     * @throws IOException if file read fail
     */
    ...
    public void advance() {
        if (hasMoreTokens()) {
            ...
            if (KEYWORDS.contains(currentToken)) currentType = TokenType.KEYWORD;
            else if (currentToken.length() == 1 && SYMBOLS.contains(currentToken.charAt(0))) currentType = TokenType.SYMBOL;
            else if (currentToken.matches("\\d+")) currentType = TokenType.INT_CONST;
            else if (currentToken.startsWith("\"")) currentType = TokenType.STRING_CONST;
            else currentType = TokenType.IDENTIFIER;
        }
    }
